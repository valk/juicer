...
9. Wire up SQUEEZER_HOME's own statusLine so opening an interactive session there shows squeezer's one-line HUD (mode/budget/TODOs/last insight) at a glance: `python3 ${CLAUDE_PLUGIN_ROOT}/daemon/install_statusline.py "$SQUEEZER_HOME" "${CLAUDE_PLUGIN_ROOT}"`. This writes `statusLine` into `SQUEEZER_HOME/.claude/settings.json` — a project-scoped settings file, which Claude Code's settings precedence ranks above the user-scoped `~/.claude/settings.json`. So it reliably shows up when a session is opened at SQUEEZER_HOME even if a plugin like claude-hud has already installed its own statusLine globally — no conflict to resolve, both keep working in their own scope.
10. Report a short summary back: SQUEEZER_HOME path, registered projects, current mode (`auto` or `human_in_loop`), confirmation the daemon service is running, and a one-line note that squeezer's HUD statusline appears in sessions opened at SQUEEZER_HOME (`cd "$SQUEEZER_HOME" && claude`) regardless of any other statusline plugin installed globally.
